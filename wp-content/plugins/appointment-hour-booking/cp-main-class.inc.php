...
class CP_AppBookingPlugin extends CP_APPBOOK_BaseClass {
    ...
    function render_status_box($name, $selected, $displayall = false) 
    {
        ...
        if ($displayall)
            echo '<option value="-1"'.($selected == '-1'?' selected':'').'>'.__('[All]','cpappb').'</option>';
        echo '<option value=""'.($selected == ''?' selected':'').'>'.__('Approved','cpappb').'</option>';
        foreach ($this->paid_statuses as $item)
            echo '<option value="'.$item.'"'.($selected == $item?' selected':'').'>'.__($item,'cpappb').'</option>';
        echo '</select>';
    }
    ...
    function insert_button() {
        ...
        if (@$_GET["action"] == 'elementor')
            print '<a href="javascript:send_to_editor(\'['.$this->shorttag.' id=&quot;1&quot;]\');" title="'.__('Insert Appointment Hour Booking','cpappb').'"><img hspace="5" src="'.plugins_url('/images/cp_form.gif', __FILE__).'" alt="'.__('Insert  Appointment Hour Booking','cpappb').'" /></a>';
        else  
